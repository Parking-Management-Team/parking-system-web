            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <span className="absolute left-3.5 top-2.5 text-slate-400 text-xs font-bold pointer-events-none">đ</span>
                  <input
                    type="number"
                    value={editedFee}
                    onChange={(e) => setEditedFee(Number(e.target.value))}
                    disabled={getNormalizedStatus(selectedIncident.status) === 'Resolved'}
                    className="w-full pl-8 pr-4 py-2 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-xs font-mono font-bold rounded-xl disabled:bg-slate-50 disabled:text-slate-400"
                  />
                </div>
              ...
              {getNormalizedStatus(selectedIncident.status) !== 'Resolved' && (
                <>
                  <button
                    ...
                    disabled={isUpdating}
                    className="px-4 py-2.5 bg-rose-50 text-rose-600 hover:bg-rose-100 text-xs font-bold rounded-xl transition-all cursor-pointer"
                  >
                    Cancel Ticket
                  </button>
                  ...
                  {getNormalizedStatus(selectedIncident.status) === 'Open' && (
                    <button
                      ...
                      disabled={isUpdating}
                      className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                    >
